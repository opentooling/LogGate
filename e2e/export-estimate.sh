...
# Pinned to this cluster: Loki also holds a remote cluster's platform-dev,
# which team ownership read from this cluster's API cannot speak for.
check "the selector is generated server-side, pinned to this cluster" \
  '{cluster="k3d-loggate", namespace="platform-dev"}' "$(body "$r" | jq_get 'd["estimate"]["selector"]')"
...
check "alice sees her own team's budget" "platform" "$(body "$r" | jq_get 'd["budgets"][0]["label"]')"
check "and only her own team" "1" "$(body "$r" | jq_get 'len(d["budgets"])')"
